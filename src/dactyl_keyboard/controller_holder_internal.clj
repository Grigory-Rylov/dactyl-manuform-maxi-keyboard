(ns dactyl-keyboard.controller-holder-internal
...
(def controllerYOffsetInternal 1)

(def controller-holder-body-internal
  (minkowski
   (cube controllerWidth, controllerLen, (+ (- totalControllerBoxHeight roundCornerHeight) 2.5))
...
;(def controller-body-x-offset (/ (- controllerCaseWidth totalControllerBoxWidth) 2))

;(def controller-body-x-offset-wireless (- (/ (- controllerCaseWidth totalControllerBoxWidth) 2) 2.7))
...
(def controllerHolderCylinderDiameterInternal 1.1)
(def controllerHolderOffsetInternal 16)
(def controllerHolderHelperInternal
  (color-red
   (translate
    [0,
     0,
     (+ 1 controllerHeight controllerBottomHeight (/ controllerHolderCylinderDiameterInternal 2))]
    (translate [0, controllerHolderOffsetInternal, 0]
               (rotate [(deg2rad 90), 0, 0]
                       (binding [*fn* 30] (cylinder (/ controllerHolderCylinderDiameterInternal 2), 3))))
    (translate [0, (- controllerLen controllerHolderOffsetInternal), 0]
               (rotate [(deg2rad 90), 0, 0]
                       (binding [*fn* 30] (cylinder (/ controllerHolderCylinderDiameterInternal 2), 3))))
    (translate [controllerWidth, controllerHolderOffsetInternal, 0]
               (rotate [(deg2rad 90), 0, 0]
                       (binding [*fn* 30] (cylinder (/ controllerHolderCylinderDiameterInternal 2), 3))))
    (translate [controllerWidth, (- controllerLen controllerHolderOffsetInternal), 0]
               (rotate [(deg2rad 90), 0, 0]
                       (binding [*fn* 30] (cylinder (/ controllerHolderCylinderDiameterInternal 2), 3)))))))

(def internal-controller-case-wired
  (union
   (difference
    (union
     ...
     (shape-insert 1, 0,
                   ...
                   controller-holder-body-internal)
                  ...
                  (union
                   ...
                   (translate
                    ...
                    (translate [0, 0, controllerBottomHeight]
                               (cube controllerWidth, (- controllerLen controllerYOffsetInternal), external-controller-height)))))
   ...
   (shape-insert 1, 0,
                 [(+ left-offset controller-body-x-offset (/ controllerWidth -2))
                  ...
                  0]
                 controllerHolderHelperInternal)
...
;           (union
;            ; click hole
;            (translate [0, 0, (/ 10 2)] (binding [*fn* fn-value] (cylinder (/ buttonClickDiameter 2) 10)))
;            (translate [0, 0, (/ (+ buttonDepth) 2)]
;                       (binding [*fn* fn-value] (cylinder (/ buttonDiameter 2) (/ controllerWallWidth 2))))
;            ;union
;            ))
;   ;switcher case hole
;   (translate
...
;       controller-body-y-offset
;       (+ totalControllerBoxHeight topOffset -0.4)]
;      controller-holder-body-internal)
;
;     ; end union
;     )))
...
;
;      (translate [0, 0, controllerBottomHeight]
;                 (cube controllerWidth, (- controllerLen controllerYOffsetInternal), (+ totalControllerBoxHeight 4))))
;     ; end union
;     )))
;
...
(def internal-controller-case-wireless
  (let [topOffset   (/ external-controller-height -2)
        holderWidth (+ buttonWidth 1)
        zOffset     -2]
    (union
...
;                     (/ totalControllerBoxHeight 2)]
;                    (translate [0, 0, controllerBottomHeight]
;                               (cube controllerWidth, (- controllerLen controllerYOffsetInternal), totalControllerBoxHeight)))))
;    ; hole end
;    )
;
;   (shape-insert 1, 0,
...
(def internal-controller-case
  (if niceNanoMode
    external-controller-case-wireless
    internal-controller-case-wired))

(def external-controller-case-nicenano-internal
  (let [topOffset   (/ external-controller-height -2)
        holderWidth (+ buttonWidth 1)
        zOffset     -2]
    ...
    (translate [0, 0, 3.5]
               (rotate [0, 0, (deg2rad 90)]
                       (union
                        (difference
                         (color-red external-case-body )
                         external-case-holes
                         (translate [0, 1, 0] (cube (+ controllerWidth 10), 5, (+ totalControllerBoxHeight 10)))
                         ;end difference

                         (translate [4, -13, 0] (binding [*fn* fn-value] (cylinder 2, 10))))

                        (color-red (translate
                         [(+ controller-body-x-offset-wireless (/ controllerWidth -2))
                          (+ controller-body-y-offset (/ controllerLen -2))
                          (- zOffset 1) ]
                         controllerHolderHelperInternal))
                        ; end union
...
(def screw-holes-internal
  (screw-insert-all-shapes-left (/ 3.6 2) (/ 3.6 2) screw-insert-height false))

(def internal-controller-plate-case
  (let [axisWidth       7.5
        axisLen         29
        verticalAxisLen 58
        height          1]
    (translate [0, 0, (/ height 2)]
               ...
               (difference
                (union
                 ; blue top
                 (translate [-30.1, 64, 0] (binding [*fn* 50] (cylinder (/ axisWidth 2), height)))
                 (translate [-30.1, 35, 0] (binding [*fn* 50] (cylinder (/ axisWidth 2), height)))
                 (translate [-30.1, 50, 0] (cube axisWidth, axisLen, height))


                 (translate [-20.1, 48, 0] (cube axisWidth, 22, height))

                 (translate [-36, 46.5, 0] external-controller-case-nicenano-internal)
                 ; end union

                 )
                (translate [-30.1, 64, 0] (binding [*fn* 50] (cylinder 1.6, (+ height 1))))
                (translate [-30.1, 35, 0] (binding [*fn* 50] (cylinder 1.6, (+ height 1))))

                (translate [-32.1, 64, 5] (cube 13 10, 9))
                (translate [-32.1, 35, 5] (cube 13 10, 9))
                ;(color-blue (translate [0, 0, -3] screw-holes-internal))
                ;end dif
                ))))
